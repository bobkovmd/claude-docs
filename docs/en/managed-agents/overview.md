...
](/docs/en/managed-agents/quickstart)[Start a session
Create a session and send your first event

](/docs/en/managed-agents/sessions)[Reference
Event types, rate limits, CLI flags, and other lookup tables

](/docs/en/managed-agents/reference)

## Core concepts

Claude Managed Agents is built around four concepts:

ConceptDescription**Agent**The model, system prompt, tools, MCP servers, and skills**Environment**Configuration for where sessions run: an Anthropic-managed cloud sandbox, or a self-hosted sandbox on your own infrastructure**Session**A running agent instance within an environment, performing a specific task and generating outputs**Events**Messages exchanged between your application and the agent (user turns, tool results, status updates)
...
- A [Claude API key](/settings/keys)

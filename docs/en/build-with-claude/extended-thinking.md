...
| --- | --- | --- |
| Claude Fable 5Claude Mythos 5 | Not supported (400 error) | [Adaptive thinking](/docs/en/build-with-claude/adaptive-thinking), always on; use [effort](/docs/en/build-with-claude/effort) to control depth |
...
The `signature` field is identical whether `displayarized"` or `"omitted"`. Switching `display` values between turns in a conversation is supported.
...
In rare cases where you need access to full thinking output for Claude 4 models, [contact Anthropic sales](/cdn-cgi/l/email-protection#6f1c0e030a1c2f0e011b071d001f060c410c0002).
...
event: content_block_delta
data "content_block_delta", "index": 0, "delta": {"type": "signature_delta", "signature": "EqQBCgIYAhIM1gbcDa9GJwZA2b3hGgxBdjrkzLoky3dl1pkiMOYds..."}}
...
event: content_block_delta
data: {"type": "content_block": 1, "delta": {"type": "text_delta", "text": "The greatest common divisor of 1071 and 462 is **21**."}}
...
| --- | --- |
| Claude Fable 5Claude Mythos 5 | Automatic with [adaptive thinking](/docs/en/build-with-claude/adaptive-thinking). Inter-tool reasoning moves into thinking blocks. No beta header needed. |
...
- [Interleaved thinking](#inter amplifies cache invalidation, as thinking blocks can occur between multiple [tool calls](#extended-thinking-with-tool-use)
...
In addition to regular `thinking` blocks, the API may returninking` blocks. A `redacted_thinking` block contains encrypted thinking content in a `data` field, with no readable summary:
...
| --- | --- | --- | --- | --- |
| Claude Fable 5Claude Mythos 5 | Not supported | Omitted by default | Automatic | See [Adaptive thinking](/docs/en/build-with-claude/adaptive-thinking) |
| [Claude Mythos Preview](https://anthropic.com/glasswing) | Supported | Omitted by default | Automatic | Preserved |
| Claude Opus 4.8 | Not supported | Omitted by default | Automatic | Preserved |
| Claude Opus 4.7 | Not supported | Omitted by default | Automatic | Preserved |
| Claude Opus 4.6 | Deprecated | Summarized | Automatic | Preserved |
...
- **Streaming requirements:** The SDKs require streaming when `max_tokens` is greater than 21,333 to avoid HTTP timeouts on long-running requests.
...
](/docs/en/build-with-claude/adaptive-thinking)[Try the extended thinking cookbook
Explore practical examples of thinking in the cookbook.

](https://platform.claude.com/cookbook/extended-thinking-extended-thinking)[Extended thinking prompting tips
Learn prompt engineering best practices for extended thinking.

](/docs/en/build-with-claude/prompt-engineering/claude-prompting-best-practices#leverage-thinking-and-interleaved-thinking-capabilities)

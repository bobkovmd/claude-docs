...
- Implement a client-side handler for each memory command. Your handler must reject paths outside `/memories`, so read [Path traversal protection](#path-traversal-protection) before you write it.
...
](/docs/en/agents-and-tools/tool-use/bash-tool)[Context editing
Automatically manage conversation context as it grows with context editing.

](/docs/en/build-with-claude/context-editing)[Compaction
Server-side context compaction for managing long conversations that approach context window limits.

](/docs/en/build-with-claude/compaction)[Tool reference

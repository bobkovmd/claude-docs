...
The memory tool is available on all Claude 4 and later models. For the full list of Anthropic-provided tools, see the Tool reference.
...
Add the memory tool to your request. The tools entry {"type": "memory_20250818", "name": "memory"} is the entire configuration: the name must be memory, and you don't define an input schema for an Anthropic-provided tool.
...
Claude's reply to a request like the previous one ends with a tool_use block that requests a memory operation, such as view /memories. Your application executes the operation and returns the result in a tool_result block, then sends the conversation back so Claude can continue: the standard tool-use loop.

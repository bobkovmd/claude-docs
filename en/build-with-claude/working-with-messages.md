...
Messages APIClaude Managed Agents**What it is**Direct model prompting accessPre-built, configurable agent harness that runs in managed infrastructure**Best for**Custom agent loops and fine-grained controlLong-running tasks and asynchronous work**Learn more**[Messages API docs](/docs/en/build-with-claude/working-with-messages)[Claude Managed Agents docs](/docs/en/managed-agents/overview)
...
[Stop reasons and fallbackHandle each `stop_reason` value and decide what to do when a response ends.
](/docs/en/build-with-claude/handling-stop-reasons)[Tool use with ClaudeGive Claude tools to call external services and APIs from within the Messages API.
](/docs/en/agents-and-tools/tool-use/overview)[Computer use toolControl desktop computer environments with the Messages API.
](/docs/en/agents-and-tools/tool-use/computer-use-tool)[Structured outputsGet guaranteed, schema-validated JSON output from Claude.
](/docs/en/build-with-claude/structured-outputs)[[icon]Task budgetsSet an advisory token budget across a full agentic loop with `output_config.task_budget`.
](/docs/en/build-with-claude/task-budgets)Was this page helpful?
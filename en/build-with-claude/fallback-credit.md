...
- 2Read two fields from the refusal
...
- 4Send the retry with the same header
...
- 1Continuation rejected: resend the unchanged body
...
- 2Token rejected: drop the token
...
[Refusals and fallbackDetect refusals and choose between server-side fallback, the SDK middleware, and a manual retry.
](/docs/en/build-with-claude/refusals-and-fallback)[Prompt cachingHow cache reads and cache writes are billed.
](/docs/en/build-with-claude/prompt-caching)[Stop reasons and fallbackEvery `stop_reason` value and how to handle it.
](/docs/en/build-with-claude/handling-stop-reasons)[SDK middlewareThe SDK helper that applies fallback credit automatically.
](/docs/en/cli-sdks-libraries/middleware)Was this page helpful?
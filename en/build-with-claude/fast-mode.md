...
ModelInputOutputClaude Opus 4.8$10 / MTok$50 / MTokClaude Opus 4.7 / Claude Opus 4.6$30 / MTok$150 / MTok
...
HeaderDescription`anthropic-fast-input-tokens-limit`Maximum fast mode input tokens per minute`anthropic-fast-input-tokens-remaining`Remaining fast mode input tokens`anthropic-fast-input-tokens-reset`Time when the fast mode input token limit resets`anthropic-fast-output-tokens-limit`Maximum fast mode output tokens per minute`anthropic-fast-output-tokens-remaining`Remaining fast mode output tokens`anthropic-fast-output-tokens-reset`Time when the fast mode output token limit resets
...
[Structured outputsGet validated JSON results from agent workflows.
](/docs/en/build-with-claude/structured-outputs)[[icon]PricingLearn about Anthropic's pricing structure for models and features.
](/docs/en/about-claude/pricing#fast-mode-pricing)[[icon]EffortControl how many tokens Claude uses when responding with the effort parameter, trading off between response thoroughness and token efficiency.
](/docs/en/build-with-claude/effort)[Streaming messagesStream Messages API responses incrementally with server-sent events, including text, tool use, and extended thinking deltas.
](/docs/en/build-with-claude/streaming)Was this page helpful?